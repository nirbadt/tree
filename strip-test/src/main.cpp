...
#define LED_COUNT 240    	//Length of your LED strip
...
CRGB local[LED_COUNT];
CRGB remote[LED_COUNT];
CRGB both[LED_COUNT];
...
uint8_t brightness = MAX_BRIGHTNESS;		//Global brightness percentage
...
void setup()
{
	// FastLED.addLeds<LED_TYPE, LED_PIN, COLOR_ORDER>(local, NUM_LEDS).setCorrection(TypicalLEDStrip);
    // FastLED.addLeds<LED_TYPE, 5, COLOR_ORDER>(remote, NUM_LEDS).setCorrection(TypicalLEDStrip);
	// FastLED.addLeds<LED_TYPE, 3, COLOR_ORDER>(both, NUM_LEDS).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<LED_TYPE, LED_PIN, COLOR_ORDER>(strip, NUM_LEDS).setCorrection(TypicalLEDStrip);
    FastLED.addLeds<LED_TYPE, 5, COLOR_ORDER>(strip, NUM_LEDS).setCorrection(TypicalLEDStrip);
	FastLED.addLeds<LED_TYPE, 3, COLOR_ORDER>(strip, NUM_LEDS).setCorrection(TypicalLEDStrip);
    FastLED.setBrightness(MAX_BRIGHTNESS);
 	FastLED.clear();
    //EEPROM.write(0,2);
    animation = EEPROM.read(0);
    EEPROM.write(0, (animation+1)%10);
}
